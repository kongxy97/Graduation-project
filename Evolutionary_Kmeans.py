...
# from ClusterCompare.Compare import Compare
...
def k_means(X, n_clusters,lastCenters,lastCategory,isFirst, init='random', precompute_distances='auto',
    ...
    if n_jobs == 1:
        ...
        print "one CPU single thread"
        for it in range(n_init):
            # run a k-means once
            labels, inertia, centers, n_iter_ = _kmeans_single(
                X, n_clusters,lastCenters=lastCenters,lastCategory=lastCategory,isFirst=isFirst, max_iter=max_iter, init=init, verbose=verbose,
        ...
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            delayed(_kmeans_single)(X, n_clusters,lastCenters=lastCenters,lastCategory=lastCategory,isFirst=isFirst, max_iter=max_iter,
...
def _centers_combine_history(lastCenters,Centers,lastCategory,labels):
    pass

def _kmeans_single(X, n_clusters,lastCenters,lastCategory,isFirst, x_squared_norms, max_iter=300,
    ...
    if isFirst:
       ...
       centers = lastCenters  
       #_init_centroids(X, n_clusters, init, random_state=random_state,
       #                       x_squared_norms=x_squared_norms)
    if verbose:
    ...
    for i in range(max_iter):
        ...
        if sp.issparse(X):
            centers = _k_means._centers_sparse(X, labels, n_clusters,
                                               distances)
            if isFirst==False:
                #compare=Compare(lastCenters,labels_old,centers,labels)
                #centers=compare.getNewCenter()
                _k_means._centers_sparse(X, labels, n_clusters,
                                               distances)
        else:
            ...
            if isFirst==False:
                #compare=Compare(lastCenters,labels_old,centers,labels)
                #centers=compare.getNewCenter()
                _k_means._centers_dense(X, labels, n_clusters, distances)
        if verbose:
...
def _labels_inertia_precompute_dense(X, x_squared_norms, centers, distances):
    ...
    n_samples = X.shape[0]
    k = centers.shape[0]
    all_distances = cosine_distances(centers, X)
...
class KMeans(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def fit(self, X, lastCenters,lastCategory,isFirst,y=None):
        ...
        self.cluster_centers_, self.labels_, self.inertia_, self.n_iter_ = \
            k_means(
                X, lastCenters=lastCenters,lastCategory=lastCategory,isFirst=isFirst, n_clusters=self.n_clusters, init=self.init,
    ...
    def fit_predict(self, X,lastCenters,lastCategory,isFirst, y=None):
        """Compute cluster centers and predict cluster index for each sample.

        Convenience method; equivalent to calling fit(X) followed by
        predict(X).
        """
        return self.fit(X,lastCenters=lastCenters,lastCategory=lastCategory,isFirst=isFirst).labels_
